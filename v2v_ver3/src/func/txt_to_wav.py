...
def get_chinese_voice_id(engine):
    """自動尋找支援中文的 voice id，找不到則回傳 None"""
    voices = engine.getProperty('voices')
    for voice in voices:
        # 常見的中文 voice id 會含有 'zh' 或 'chinese'
...
def txt_to_wav(
    input_path: str,
    output_path: str = None,
    voice_id: str = None,  # Linux 下不建議預設值，需自動尋找
    rate: int = 150,
    volume: float = 0.9,
    encoding: str = "utf-8"
) -> None:
    ...
    try:
        # 自動尋找中文 voice
        if voice_id is None:
            voice_id = get_chinese_voice_id(engine)
            if voice_id is None:
                print("警告：未找到中文語音，將使用預設語音。")
...
if __name__ == "__main__":
    # 基本用法 (自動檢測編碼)
    txt_to_wav(
        input_path="/home/yourname/input/test.txt",
        output_path="/home/yourname/output/SB.wav",
        # encoding="big5"
    )
