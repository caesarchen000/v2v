...
SERVER_URL = 'http://10.10.32.71:5000'  # 換成你的伺服器IP
...
def connect():
    global connected
    connected = True
    print(f"[Connected to Server] {CAR_ID}")
...
###sender###
def send_txt_to_server_and_store_json(self_id: str, message, mode:str):
    """
    打包sending thing to a one object -> dict
    data is the package
    mode should be 'sender' or 'receiver'
    """
    data = {
        "target_id": self_id,
        "aim": "txt to json", 
...
def send_json_to_server_and_store_txt(self_id: str, message, mode:str):
    """
    打包sending thing to a one object -> dict
    data is the package
    mode should be 'sender' or 'receiver'
...
def tell_server_correctness(self_id:str, mode:str, message, content:str):
    data={
        "target_id": self_id,
        "aim": "tell correctness",
        "mode": mode,
        "payload": message,
        "content": content   
    }
...
def send_for_confirm(self_id:str, message, mode:str):
    data = {
        "target_id": self_id,
        "aim": "confirm", 
        "mode": mode,
