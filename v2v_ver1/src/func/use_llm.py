...
# Load the model
llama3 = Llama(
    model_path = "/home/jason/Downloads/Llama-3.2-1B.Q4_K_M.gguf",
    verbose=False,
...
def txt_to_json_pipeline(txt_message: str) -> json:
    
    extracted_keywords = keyword_extraction_agent.inference(txt_message)
...
def json_to_txt_pipeline(json_message:json) -> str:

    formatted_message = f"""
    1. 來自的車牌號碼：{json_message.get("來自的車牌號碼", "none")}
    2. 傳達訊息：{json_message.get("傳達訊息", "none")}
    """
    reconstructed_message = json_to_txt_agent.inference(formatted_message)
    return reconstructed_message

def confirm_pipeline(confirm_msg: str) -> str:
