...
###connection###
from makeNTU.v2v.src.server_client.new_client import *
from makeNTU.v2v.src.server_client.new_server import *
###display###
...
def sender_pipeline() -> bool:
    ...
    ### send to server to confirm
    with open('sender_tmp/sender_final_confirm.txt', 'r', encoding='utf-8') as f:
        content = f.read() 
        send_for_confirm(CAR_ID,content,mode='sender_tmp/sender_final_confirm_reply.txt')
        with open('sender_tmp/sender_final_confirm_reply.txt', 'r', encoding='utf-8') as f2:
            content2=f2.read()
            sender_final_check=content2
...
def sender_pipeline_part1():
    ...
    with open('sender_tmp/sender.txt', 'r', encoding='utf-8') as f:
        ...
        # use LLM to convert text to json
        ### send to server to use llm to get json from txt ###
        send_txt_for_json(CAR_ID,content,mode="sender_tmp/sender.json")
...
def sender_pipeline_part2():
    ...
    ### send to server to get txt from json ###
    with open('sender_tmp/sender.json', 'r', encoding='utf-8') as f:
        content = f.read()
        send_json_for_txt(CAR_ID,content, mode='sender_tmp/sender_notyet_confirm.txt')
...
def handle_incoming(data_pack):
    ...
    if aim == 'txt to json' or 'json to txt':
        filepath=mode 
    elif aim == 'transport':
...
def receiver_pipeline() -> bool:
    ...
    with open('receiver_tmp/received.json', 'r', encoding='utf-8') as f:
        content = f.read()
        send_json_for_txt(CAR_ID,content, mode='receiver_tmp/received.txt')
    # convert text to voice
    ...
    with open('receiver_tmp/receiver_final_confirm.txt', 'r', encoding='utf-8') as f:
        content = f.read() 
        send_for_confirm(CAR_ID,content,mode='receiver_tmp/receiver_final_confirm_reply.txt')
        with open('receiver_tmp/receiver_final_confirm_reply.txt', 'r', encoding='utf-8') as f2:
            content2=f2.read()
            receiver_final_check=content2
    # use LLM to convert text to json
...
def receiver_pipeline_part1():
    ...
    with open('receiver_tmp/receiver.txt', 'r', encoding='utf-8') as f:
        ...
        # use LLM to convert text to json
        ### send to server to get json from txt ###
        send_txt_for_json(CAR_ID,content,mode="receiver_tmp/receiver.json")
...
def receiver_pipeline_part2():
    ...
    with open('receiver_tmp/receiver.json', 'r', encoding='utf-8') as f:
        content = f.read()
        send_json_for_txt(CAR_ID,content, mode='receiver_tmp/receiver_notyet_confirm.txt')
